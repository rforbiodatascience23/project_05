...
```{r}
# Plotting a nice volcano plot just for visualization purpose (that is why we get rid of the extrem FC values and we only take 2000 samples)
...
model_estimates |>
  left_join(fc_change, by=join_by(gene)) |>
  filter(log2(FC_change)>-5 & log2(FC_change)<5) |>
...
```{r}
# Putting the whole dataset on the volcano plot + the filter for low expression difference
...
ggsave("../pictures/volcano_plot_total.png", bg="white")
```
