...
meta_gene_table <- meta_gene_table |> 
  dplyr::select(-one_of(zero_expression_genes))
```
...
write_tsv(model_estimates, 
          file = "../data/gene_model_estimates.tsv")
```
...
We ended up with using the lose approach, as no genes satisfied the strict approach
...
write_tsv(filtered_sig_data, 
          file = "../data/cleaned_meta_gene_table.tsv.gz")
```
