...
**Save yourself some time:** The result of this time-consuming analysis, is there are no columns containing missing values.
...
Find the names of columns that contain missing values.
...
Find number of genes before removing genes with non-significant change in expression between lesional and perilesional samples
...
Create a categorical variable that classifies genes with significant change in expression level. Genes with adj. p-val below 0.05 are categorized as significant.
...
Find the names and number of genes with significant change in expression
...
Remove the non-significant columns
...
```{r}
# save the full modelled data before significant gene selection
write_tsv(model_estimates, 
          file = gzfile("../data/gene_model_estimates.tsv.gz"))
```

##  Remove genes that are less than 3-fold up- or down regulated
...
We now subset gene with at least 3-fold change in expression. We used 2 filtering approach:

-   **Strict** - where the gene must have 3-fold change across all sample.

-   **Loose** - the gene only need to have 3-fold change across 1 sample

We ended up with using the loose approach, as no genes satisfied the strict approach

```{r}
# Find count before filtering
...
# Using 2 filtering approach
# Strict, the gene must have 3-fold change across all sample
# Lose, the gene only need to have 3-fold change across 1 sample
...
```{r}
# Applying filtering loss, and remove genes with less that 3-fold change in expression
filtered_sig_data <- gene_expr_diff |> 
  filter(gene %in% high_diff_lose) 
```
