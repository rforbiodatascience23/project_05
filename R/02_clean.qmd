...
# get genes names that have 0 expression (6098 genes are found to be 0 expressed)
...
```{r}
# select non 0 expression genes from the data (6098 genes are deleted)
meta_gene_table <- meta_gene_table |> 
  select(-all_of(zero_expression_genes))
```

```{r}
# Creating a tibble with only the genes columns
gene_cols <- meta_gene_table |> 
...
gene_number_unfiltered <- length(gene_cols)

gene_expr_diff <- meta_gene_table |>
...
filtering_res <- data.frame(
            unfiltered = gene_number_unfiltered, 
...
# significant gene epxpression difference when adjuste p-value > 0.01
...
model_res <- data.frame(after_filtering = length(high_diff_lose),
